        ...
        slide.querySelectorAll("[data-action]").forEach((btn) => {
            btn.addEventListener("click", (e) => {
                ...
                if (action === "start" || action === "next" || action === "submit") goNext();
                    ...
                    goTo(state.slides.findIndex((s) => s.dataset.key === "squadSize"));
                }
            });
        });
    ...
    state.slides.forEach((slide) => {
        ...
        if (type === "text" || type === "email") {
            ...
            choices.forEach((c) => {
                ...
                if (
                    typeof val === "string" &&
                    (val === cv || (val.startsWith("Other:") && c.dataset.other === "true"))
                ) {
            ...
            if (Array.isArray(val)) {
                slide.querySelectorAll(".choice").forEach((c) => {
                    ...
                    const isSelected =
                        val.includes(cv) ||
                        (isOther && val.some((v) => typeof v === "string" && v.startsWith("Other:")));
                    if (isSelected) {
                        c.classList.add("is-selected");
                        if (isOther) {
                            const oi = slide.querySelector("[data-other-input]");
